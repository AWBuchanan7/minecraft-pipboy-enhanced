...
public class InvoScrollBar extends GuiButton {
				...
				if (s.getClass() != Slot.class && s.getClass() != SlotLockable.class) {
					InfiniteInvo.logger.log(Level.WARN,
							"Container " + container.getClass().getSimpleName()
									+ " is not supported by InfiniteInvo! Reason: Custom Slots (" + s.getClass()
									+ ") are being used!");
					return false;
				}
	...
	public void drawButton(Minecraft mc, int mx, int my) {
		...
		if (maxScroll > 0) {
			...
			this.drawTexturedModalRect(this.guiLeft + scrollX + 20,
		...
		if (Minecraft.getMinecraft().thePlayer.inventory instanceof BigInventoryPlayer) {
			...
			for (int i = 0; i < this.invoSlots.length; i++) {
				...
				// this.drawString(mc.fontRenderer, "" + s.getSlotIndex(), s.xDisplayPosition +
				// this.guiLeft, s.yDisplayPosition + this.guiTop + 4, 16777120);
				// mc.getTextureManager().bindTexture(new ResourceLocation("infiniteinvo",
				// "textures/gui/adjustable_gui.png"));
				// GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
			}
		}
	}
